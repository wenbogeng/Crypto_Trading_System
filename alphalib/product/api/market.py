...
from alphalib.product.utils.commons import robust, robust_
...
def fetch_binance_swap_candle_data(exchange, symbol, run_time, limit=1000):
    try:
        ...
        # 整理数据
        df['candle_begin_time'] = pd.to_datetime(df['candle_begin_time'], unit='ms') + pd.Timedelta(
...
def fetch_fundingrate(exchange):
    ...
    df['candle_begin_time'] = df['candle_begin_time'].apply(lambda x: pd.to_datetime(
        x.to_pydatetime().replace(minute=0, second=0, microsecond=0).strftime("%Y-%m-%d %H:%M:%S")))
    utc_offset = int(time.localtime().tm_gmtoff / 60 / 60)
