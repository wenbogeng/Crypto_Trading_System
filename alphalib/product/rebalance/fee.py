...
def spot_buy_quote2(exchange, symbol, quote_amount):
    ticker = exchange.public_get_ticker_price(params={'symbol': symbol})
    price = float(ticker['price'])
    quantity = quote_amount / price * 0.98  # 留一些余量
...
def spot_buy_limit_quote(exchange, symbol, quote_amount):
    ticker = exchange.public_get_ticker_price(params={'symbol': symbol})
    price = float(ticker['price']) * 1.01
...
def replenish_bnb(exchange, balance, amount_t=10):
    ...
    if amount_bnb < min_bnb:
        ...
        if spot_bnb_amount < min_bnb:
            ...
            spot_buy_quote2(exchange, 'BNBUSDT', amount_t)
