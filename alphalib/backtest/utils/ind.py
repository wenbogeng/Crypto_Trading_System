...
def cal_ind(_select_c):
    ...
    results.loc[0, '盈亏收益比'] = round(
        select_coin.loc[select_coin['本周期多空涨跌幅'] > 0]['本周期多空涨跌幅'].mean() /  # noqa: W504
    ...
    #    '1 days 00:00:00' / (select_coin['candle_begin_time'].iloc[-1] - \
    #          select_coin['candle_begin_time'].iloc[0]) * 365) - 1
